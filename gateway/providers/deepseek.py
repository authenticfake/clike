...
_RESPONSES_URL = "https://api.deepseek.com/v1/responses"
...
async def chat(
    ...
    if "code" in model:
        gen["response_format"] =None
        gen["tools"] = None
        gen["tool_choice"] = None
    ...
    return await deepseek_complete_unified(api_key=api_key, model=model, messages=messages, gen=gen, timeout_s=timeout)

# --- end: response normalizers ---
async def deepseek_complete_unified(
    ...
    log.info("deepseek_complete_unified resonseAPi %s, payload %s", use_responses, payload)
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout_s) as client:
                ...
                log.info("deepseek_complete_unified response %s", r.status_code)
                #LOG RESPONSE OPENAI 
                #log.info("gateway._post_with_retries response text %s", r.text)
       
    except Exception as e:
        log.error("exception deepseek_complete_unified Error: %s", e, exc_info=True)
    ...
    log.info("after calling...deepseek_complete_unified: %s", r.status_code)
    # Successo 200 → normalizza e ritorna
    if r.status_code == 200:
        try:
            log.info("deepseek_complete_unified text: %s", r.text)
            return normalizer(r.json())
        except Exception as e:
            log.error("deepseek_complete_unified normalizer: %s", e) 
    ...
    log.error("deepseek_complete_unified text: %s", r.text)
    try:
        j = r.json()
        err = j.get("error", {}) if isinstance(j, dict) else {}
    except Exception:
        log.error("deepseek_complete_unified: %s", r)
        err = {}
