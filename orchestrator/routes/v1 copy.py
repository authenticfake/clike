...
# === Structured Outputs schema for CLike files ===
# === Structured Outputs schema for CLike files (OpenAI strict) ===
FILES_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "files": {
        ...
        "messages": {
            ...
            "items": {
                ...
                "additionalProperties": False   # <— hardening (prima era True o assente)
            }
        }
    },
    "required": ["files"],
    "additionalProperties": False               # <— FIX principale (root)
}
...
async def chat(req: Request):
    ...
    log.info("chat request: %s", json.dump(body, f, ensure_ascii=False, indent=2))
    ...
    try:
        ...
        log.info("chat response: %s", json.dump({"text": text}, f, ensure_ascii=False, indent=2))
...
async def generate(req: Request):
   ...
   # sysmsg = {"role": "system", "content": _system_for_generate(mode)} if ' _system_for_generate' in globals() else sys_schema
    ...
    log.info("gen request: %s", json.dump(body, f, ensure_ascii=False, indent=2))
    ...
    try:
        raw = await llm_client.call_gateway_chat(
            ...
            response_format=response_format   # <<— QUI la differenza
        )
    ...
    log.info("explanation: %s", assistant_text) 
    ...
    log.info("response: %s", json.dump(resp, f, ensure_ascii=False, indent=2))
    log.info("files.json: %s", json.dump(files_planned, f, ensure_ascii=False, indent=2))
...
async def apply(req: Request):
    ...
    for fobj in files:
        ...
        log.info("apply_debug",json.dump({"applied": audit_apply, "failures": failures}, fa, ensure_ascii=False, indent=2) )
